...
do {
  let wp: WhatsappService = new WhatsappService();
  ...
  switch (opc) {
    ...
    case 0:
      break;
    case 1:
      let nome: string = readline.question('Nome do user: ');
      wp.addUser(nome);
      break;
    case 2:
    ...
    case 3:
      ...
      5. Sair`);
        ...
        switch (op) {
          case 1:
            let chat: string = readline.question('Nome do grupo: ');
          ...
          6. Voltar para menu usuario`);
              ...
              switch (o) {
                case 1:
                  let userInvited: string = readline.question('Convidar: ');
                ...
                case 2:
                  ...
                  break;
                case 3:
                  let text: string = readline.question('Mensagem: ');
                  wp.getRepUser().map(user => {
                    if (user.getUsername() === name) {
                      wp.getRepChat().map(group => {
                        if (group.getName() === chat) {
                          if (
                            group.deliverZap(user.sendMsg(chat, text)) === true
                          ) {
